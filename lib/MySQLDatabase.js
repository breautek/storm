...
class MySQLDatabase extends Database_1.Database {
    ...
    _addNode(nodeID, config) {
        instance_1.getInstance().getLogger().trace(`Adding node to connection pool: "${nodeID}"`);
        this.cluster.add(nodeID, config);
    }
    _removeNode(nodeID) {
        instance_1.getInstance().getLogger().trace(`Removing node to connection pool: "${nodeID}"`);
        this.cluster.remove(nodeID);
    }
    _getConnection(query, requireWriteAccess) {
        instance_1.getInstance().getLogger().trace(`Querying connection pool for "${query}".`);
        return new Promise((resolve, reject) => {
            this.cluster.getConnection(query, (error, connection) => {
                ...
                resolve(new MySQLConnection_1.MySQLConnection(connection, !requireWriteAccess));
            });
        });
    }
}
