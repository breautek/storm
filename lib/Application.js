...
class Application extends EventEmitter {
	...
	attachHandler(path, Handler) {
		this._server.get(path, (request, response) => {
			new Handler(this).get(new Request(request), new Response(response));
		});
		this._server.post(path, (request, response) => {
			new Handler(this).post(new Request(request), new Response(response));
		});
		this._server.delete(path, (request, response) => {
			new Handler(this).delete(new Request(request), new Response(response));
		});
		this._server.put(path, (request, response) => {
			new Handler(this).put(new Request(request), new Response(response));
		});
	}
