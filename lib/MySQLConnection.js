...
const startTransactionQuery = new StartTransactionQuery_1.StartTransactionQuery();
const commitQuery = new CommitQuery_1.CommitQuery();
const rollbackQuery = new RollbackQuery_1.RollbackQuery();
class MySQLConnection extends DatabaseConnection_1.DatabaseConnection {
    ...
    _stream(query, params, streamOptions) {
        ...
        const queryObject = this.getAPI().query({
            sql: query,
            timeout: this.getTimeout()
        }, params);
        instance_1.getApplicationLogger().trace(queryObject.sql);
