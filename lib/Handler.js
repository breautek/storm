...
class Handler {
    constructor(app) {
        ...
        if (this._middlewares.length > 0) {
            instance_2.getApplicationLogger().warn(new Error('Handler middlewares is deprecated and will be removed in the future.').stack);
        }
    }
    initMiddlewares() {
        return [];
    ...
    _executeMiddlewares(request, response, index = 0) {
        return new Promise((resolve, reject) => {
            let promises = [];
            let firedDeprecation = false;
            for (let i = 0; i < this._middlewares.length; i++) {
                if (!firedDeprecation) {
                    instance_2.getApplicationLogger().warn(new Error('Handler._executeMiddlewares is deprecated. Will be removed in the future.').stack);
